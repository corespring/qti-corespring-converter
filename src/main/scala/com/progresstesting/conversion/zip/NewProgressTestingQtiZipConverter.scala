...
import org.corespring.conversion.qti.manifest.{ManifestItem, ManifestResource, ZipReader, ZipWriter}
...
object NewProgressTestingQtiZipConverter
  ...
  val logger = LoggerFactory.getLogger(NewProgressTestingQtiZipConverter.this.getClass)
  ...
  def convert(
    ...
    def toSourceMap(resources: Seq[ManifestResource]) : Map[String, SourceWrapper] = {
      resources.map{ r =>
    ...
    def toCorespringItem(m:ManifestItem) : Future[Option[CorespringItem]] = Future{
      ...
      qti.map{ q =>
        val sources : Map[String, SourceWrapper] = toSourceMap(m.resources)
